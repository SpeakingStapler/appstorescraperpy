...
class AppStoreScraper:
    ...
    def __get(
        ...
        with requests.Session() as s:
            s.mount(AppStoreScraper.__base_request_url, HTTPAdapter(max_retries=retries))
    ...
    def __build_review_url(country,app_id):
        ...
        return f"{AppStoreScraper.__base_request_url}/{request_path}"
    
    def __build_landing_url(country,app_id):
        landing_path = f"{country}/app/id{app_id}"
        landing_url = f"{AppStoreScraper.__base_landing_url}/{landing_path}"
    ...
    def __build_review_header(landing_url,token):
        return {
            ...
            "Origin": AppStoreScraper.__base_landing_url,
            "Referer": landing_url,
            "User-Agent": random.choice(AppStoreScraper.__user_agents),
        }
    
    def __get_review_token(landing_url):
        res = AppStoreScraper.__get(landing_url)
        ...
        scriptjs = AppStoreScraper.__get(f"{AppStoreScraper.__base_landing_url}{script_path.group(1)}")
    ...
    def __get_app_reviews_per_country(app_id, country=None, count:int = None,offset:int = 0, sleep:int = 0.3,headers = None):
        ...
        review_url = AppStoreScraper.__build_review_url(country,app_id)
        ...
        while True:
            ...
            res = AppStoreScraper.__get(review_url,
                            headers,
                            request_params)
            
            # parse the review data and append to reviews list
            reviews.extend(AppStoreScraper.__parse_review_data(res))

            # get the next offset number
            request_offset = AppStoreScraper.__get_next_offset(res)
    ...
    def _check_review_availability(app_id,country,headers = None):
        ...
        avail_url = AppStoreScraper.__build_review_url(country,app_id)
    ...
    def _get_app_reviews(app_id,count:int = None,sleep:int = 0.3):
        ...
        for country in pycountry.countries:
            ...
            landing_url = AppStoreScraper.__build_landing_url(country.alpha_2.lower(),app_id)
            ...
            res,offset = AppStoreScraper.__get_app_reviews_per_country(app_id,country.alpha_2.lower(),count,offset,sleep,headers)
            reviews.extend(res)
